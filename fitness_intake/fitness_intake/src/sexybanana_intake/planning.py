...
def plan_contract(
    ...
    return {
        ...
        "detail_requirements": {
            "training_day": ["title", "focus"],
            "every_block": ["completion_rule", "load_type", "coaching_cues"],
            "strength_main_block": [
                "sets",
                "repetitions",
            ...
            "numeric_external_load_forbidden": True,
        },
    }
...
def validate_candidate(
    ...
    for day in candidate.days:
        ...
        if day.kind != "training":
            ...
            continue
        if not day.title or not day.focus:
            raise IntakeError(
                "invalid_plan", "Training days require a title and at least one focus label."
            )
        ...
        for block in day.blocks:
            if (
                block.completion_rule is None
                or block.load_type is None
                or not block.coaching_cues
            ):
                raise IntakeError(
                    "invalid_plan",
                    "Every exercise block needs a completion rule, load type, and coaching cues.",
                )
            ...
            if activity["category"] == "strength" and block.phase == "main":
                ...
                if (
                    block.completion_rule != "repetitions"
                    or block.rest_seconds_between_sets is None
                    or block.tempo_seconds_per_rep is None
                    or block.rest_seconds_between_sets * max(block.sets - 1, 0)
                    > block.rest_minutes * 60 + 1e-8
                ):
                    raise IntakeError(
                        "invalid_plan",
                        "Strength blocks need an explicit repetition tempo and between-set rest.",
                    )
            elif block.completion_rule != "duration":
                raise IntakeError(
                    "invalid_plan", "Non-strength blocks must use duration completion."
                )
...
def finalize(
    ...
    for day in candidate.days:
        ...
        item["objective"] = (
            item.get("title") or "Practice tolerated movement and observe recovery."
            if day.kind == "training"
...
def render_plan_markdown(plan: dict) -> str:
    ...
    for day in plan["days"]:
        ...
        if day["blocks"]:
            lines += [
                "",
                "| Phase | Activity | Work | Rest | Load | RPE (0–10) |",
                "| --- | --- | --- | --- | --- | ---: |",
            ]
            for b in day["blocks"]:
                dose = (
                    f"{b['sets']} × {b['repetitions']} at {b['tempo_seconds_per_rep']:g} sec/rep"
                    if b["sets"]
                    else f"{b['active_minutes']:g} min"
                )
                rest = (
                    f"{b['rest_seconds_between_sets']} sec between sets"
                    if b.get("rest_seconds_between_sets") is not None
                    else f"{b['rest_minutes']:g} min"
                )
                lines.append(
                    f"| {b['phase']} | {safe(b['name'])} | {dose} | {rest} | {safe(b['load_type'].replace('_', ' '))} | {b['rpe']:g} |"
                )
            lines += [
                "",
                *[
                    f"- **{safe(b['name'])}:** {safe(b['instructions'])} Cues: "
                    + "; ".join(safe(cue) for cue in b.get("coaching_cues", []))
                    for b in day["blocks"]
                ],
            ]
        if day["day"] in [7, 14]:
